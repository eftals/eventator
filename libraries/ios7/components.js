	...
	,editableProperties: [
		...
		},
		{
			...
			,changeProperty: {
				...
				property: 'text',
				selector: '.ios7-navbar-button-left',
				transitionable: false
			}
		},
		{
			...
			,changeProperty: {
				...
				property: 'text',
				selector: '.ios7-navbar-button-right',
				transitionable: false
			}
		}
	]
...
prx.types.ios7_pagecontroller = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		$.each(item.buttons, function(i,elm) {
			cR += '<div class="ios7-pagecontroller-button dynamic-property" data-dynamic-property-index="'+i+'">';
			cR += '<input type="radio" name="'+_id+'-radio-input" id="'+_id+'-radio-'+i+'" '+((item.selected == i) ? 'checked' : '')+' data-role="none" '+((item.active == i) ? 'class="liveUpdate-activeBackgroundColor liveUpdate-activeTextColor"' : '')+' ' +((!item.changeActive) ? 'disabled' : '')+ '>'
			cR += '<label id="'+_id+'-buttons-'+i+'" for="'+_id+'-radio-'+i+'" class="changeProperty-backgroundColor changeProperty-borderRadius '+((item.selected == i) ? 'liveUpdate-activeBackgroundColor' : 'liveUpdate-backgroundColor')+'" ></label>';
			cR += '</div>';
		});
		cR += '</div>'
		cR += '</div>';
...
prx.types.ios7_listcomplex = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '#' + _id + ' label:active { background: '+getColor(item.activeBackgroundColor)+'; color: '+getColor(item.activeTextColor)+'; }'
		cR += '#' + _id + ' label:active .ios7-list-value { color: '+getColor(item.activeValueColor)+'; }'
		cR += '#' + _id + ' label:active .ios7-list-subtitle { color: '+getColor(item.activeSubColor)+'; }'
		cR += '</style>'
		...
		$.each(item.listitems, function(i,elm){		
			...
			if(elm.itemtype == 'checkbox' || elm.itemtype == 'radio') {
				cR += '<input type="'+((elm.itemtype == 'checkbox') ? 'checkbox': 'radio') +'" name="'+_id+'-checkbox" id="'+_id+'-checkbox-'+i+'" data-role="none" '+ ((elm.checked) ? 'checked' : '') +' />';
			}
...
prx.types.ios7_list_header = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<div class="ios7-list-header-inner liveUpdate-backgroundColor liveUpdate-textColor liveUpdate-borderColor changeProperty-backgroundColor changeProperty-textColor changeProperty-textSize changeProperty-textFont">';
